<template>
  <div class="publisher-container">
    <div class="publisher-header">
      <h1>{{ publisherName }}</h1>
      <p class="publisher-subtitle">发行的游戏</p>
    </div>
    ...
    <div v-else-if="publisherGames.length === 0" class="no-games">
      <p>该发行商暂无游戏</p>
...
export default defineComponent({
  ...
  setup() {
    ...
    // 根据发行商筛选游戏
    const publisherGames = computed(() => {
      if (!publisherName.value) return []
      return gameStore.games.filter(game => 
        game.publisher === publisherName.value
      )
    })
    ...
    const navigateToGameDetail = (gameId: number) => {
      router.push(`/game/${gameId}?from=publisher&publisher=${encodeURIComponent(publisherName.value)}`)
    }
...
.game-type, .game-developer, .game-release {
  margin: 0.5rem 0;
  font-size: 0.9rem;
  color: #8f98a0;
}
